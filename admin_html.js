...
select.addEventListener('change', function(e){
	...
	if(op=='Chọn đáp án'){
		...
		submit.onclick = function(){
			...
			let listQs = JSON.parse(localStorage.getItem('listQs')) || [];
			 //cập nhật dữ liệu
			listQs.push(qsindex);
			//lưu dữ liệu mới vào local
			localStorage.setItem('listQs', JSON.stringify(listQs));
...
function updateTable() {
	...
	table.innerHTML = '<tr style="background: purple; font-weight: bold; color: white;"><td>STT</td><td><p>Câu hỏi</p></td><td>Loại câu hỏi</td><td>Người thêm</td><td>Thời gian thêm</td><td>Trạng thái</td><td>Chỉnh sửa</td><td>Xóa</td><td>Xem chi tiết</td><td>Duyệt</td></tr>';

	//lấy dữ liệu từ local
	let listQs = JSON.parse(localStorage.getItem('listQs')) || [];

	for (let i = 0; i < listQs.length; i++) {
            const qsindex = listQs[i];
            ...
            function editQuestion(index) {
   		 		var listQs = JSON.parse(localStorage.getItem('listQs')) || [];
    			if(listQs[index].status=="Chờ duyệt"){
    			var qsnew = listQs[index].answers;
    			...
    			listQs[index].correct = newcorrect;
    			listQs[index].nameqs = newText;
    			listQs[index].answers = newAnswers;
            ...
            function deleteQuestion(index) {
        		const listQs = JSON.parse(localStorage.getItem('listQs')) || [];
        		listQs.splice(index, 1);
        		localStorage.setItem('listQs', JSON.stringify(listQs));
        		updateTable();
    		}
            ...
            function appQuestion(index) {
            	var listQs = JSON.parse(localStorage.getItem('listQs')) || [];
    			listQs[index].status = "Đã duyệt";
    			console.log(listQs[index].status);
    			localStorage.setItem('listQs', JSON.stringify(listQs));
    			updateTable();
            }
...
function seeQuestion(index) {
    ...
    var listQs = JSON.parse(localStorage.getItem('listQs')) || [];
    var op = listQs[index].op;
    if(op == 'Chọn đáp án'){
        var listQs = JSON.parse(localStorage.getItem('listQs')) || [];
        var seeqs = [];
        seeqs = listQs[index].answers;
        cauhoi.innerHTML = ' ';
        dapandung.innerHTML = ' ';
        dapan.innerHTML = ' ';
        var them = ' ';
        cauhoi.innerHTML ='Câu hỏi: ' + listQs[index].nameqs;
        for(i = 1; i <= seeqs.length; i++){
            them= them + '<div>' + i + ")      " + seeqs[i-1] + '</div>';
        }
        dapan.innerHTML = them;
        dapandung.innerHTML = 'Đáp án đúng: ' + listQs[index].correct; 
    }
    else{
        dapan.innerHTML = ' ';
        dapandung.innerHTML = ' ';
